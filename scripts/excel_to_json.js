...
const CONSEJO_COL_MAP = {
  ...
  "PROMEDIO DE ALUMNOS NECESARIOS POR SESIÓN":          ["pres_prom",   "float"],
  "PRES MIN":                                           ["pres_min",    "int"],
  "MÍNIMO DE SESIONES NECESARIAS":                      ["pres_min",    "int"],
  "PRES NECES":                                         ["pres_neces",  "int"],
  "NECESARIAS A PLANIFICAR":                            ["pres_neces",  "int"],
...
function parseEvidencias(ws) {
  ...
  for (const row of rows) {
    ...
    byConsejo[consejo][sesion].push({ nombre: evidencia, nota });
  }
  return byConsejo;
}
